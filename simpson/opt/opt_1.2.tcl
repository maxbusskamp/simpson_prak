...
# modified version, p.selter 07/17
# makes use of simulated annealing instead of downhill (hopefully...)

# modified version m_gerd 06/18
# changes the annealing algorithm to have all parameters as arguments with arbitrary defaults
# also has maximum number of iterations per temperature step as argument

package provide opt 1.2 
...
# parse all options into the function (opposed to the 1.1 functionality that uses the set functions) with arbitrary defaults // m_gerd 2018-12-06
# this makes iter available to changes, which is the maximum iteration count per cooling step (total number of steps is iter*t_step+1)
# tolerance, maxiteration , starting Temp, number of cooling steps, exponent of the Temp curve, reset frequency to current maximum
proc ::opt::annealing {{tol 1.0e-3} {iter 1000} {param_temp 10} {param_t_step 5} {param_Texpt 0.25} {param_rfreq 1}} {
	::opt::temp $param_temp
	::opt::t_step $param_t_step
	::opt::texponent $param_Texpt
	::opt::restartfreq $param_rfreq
	set n [llength $::opt::_internal::parameters]
	for {set j -1} {$j < $n} {incr j} {
		if [info exists row] {unset row}
		for {set i 0} {$i < $n} {incr i} {
	...
	set optimizer [optimization -annealing -T $::opt::_internal::temp -Tsteps $::opt::_internal::t_step -Texpt $::opt::_internal::texp -restartfreq $::opt::_internal::rfreq -simplex $simplex -function ::opt::_internal::wrapper]
	$optimizer configure -tol $tol
	$optimizer configure -iter $iter
	set result [$optimizer start]
	if {[llength $result] == 2} {
		for {set i 0} {$i < $n} {incr i} {
			set nam [lindex $::opt::_internal::parameters $i]
...
proc ::opt::confidence_annealing {parname} {
	set i [lsearch $::opt::_internal::parameters $parname]
	if {$i < 0} {
		puts stderr "Cannot calculate confidence interval for parameter $parname"
		return
...
proc ::opt::temp {temperature} {
	set ::opt::_internal::temp $temperature
}

proc ::opt::t_step {t_step} {
	set ::opt::_internal::t_step $t_step
}

proc ::opt::texponent {texp} {
	set ::opt::_internal::texp $texp
}

proc ::opt::restartfreq {rfreq} {
	set ::opt::_internal::rfreq $rfreq
}
